...
<script type="text/javascript">
    window.onload = function () {
        document.getElementById("bt1").onclick = function () {
            ...
            //03.发送请求
            xhr.open("get",
                "/client/ClientServlet?op=addCart&book_id="
                + book_id);
            //04.发送请求正文
            xhr.send(null);
            alert("添加成功!");
        ...
        document.getElementById("a1").onclick = function () {
            ...
            //03.发送请求
            xhr.open("get",
                "/client/ClientServlet?op=addfavorite&book_id="
                + book_id);
            //04.发送请求正文
            xhr.send(null);
        }
...
<div
    ...
    <div style="width: 640px; height: 600px; float: right;">
        ...
        <div style="margin-left: 18px; margin-top: 15px">
            <span style="font-size: 20px; color: #1890FF;">${book.book_press}</span>
            &nbsp;<strong>编</strong>&nbsp;&nbsp;&nbsp; <span
                style="font-size: 20px; color: #1890FF;">${book.book_author}</span>
            &nbsp;<strong>著</strong>
        </div>
        <!-- 价格&销量 -->
        <div
        ...
        <div style="margin-top: 60px; margin-left: 120px">
            ...
            <button id="bt2" type="button" class="btn btn-danger"
                    style="background-color: rgb(198, 46, 45); width: 160px; height: 43px; margin-left: 100px">
						<span style="font-size: 16px " ><a href="<%=basePath%>/client/ClientServlet?op=buyNow&book_id=${book.book_id}">立即购买</a></span>
                                </button>
             </c:if>
            <c:if test="${empty user}">
...
<script type="text/javascript">
    ...
    function buyNow() {
        ...
        //03.发送请求
        xhr.open("get",
            "/client/ClientServlet?op=buyNow&book_id="
            + book_id);
        //04.发送请求正文
        xhr.send(null);
    }
